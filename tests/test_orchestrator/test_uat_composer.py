...
class TestComposeRemediationInputTracks:
    def test_lists_each_track_name_when_provided(self) -> None:
        tracks = [
            Track(name="web", kind="web", uat_kind="web"),
            Track(name="api", kind="api", uat_kind="api"),
        ]
        result = compose_remediation_input("report", "my-app", tracks=tracks)
        assert "web" in result
        assert "api" in result

    def test_falls_back_to_generic_existing_without_tracks(self) -> None:
        result = compose_remediation_input("report", "my-app")
        assert "existing" in result

    def test_empty_track_list_falls_back_to_existing(self) -> None:
        result = compose_remediation_input("report", "my-app", tracks=[])
        assert "existing" in result
