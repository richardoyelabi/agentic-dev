...
# ---------------------------------------------------------------------------
# allow_empty_action_output (for code-writing agents)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_allow_empty_action_output_skips_empty_check(
    claude, action_agent, qa_agent, doc_store, prompt_renderer
):
    """When allow_empty_action_output=True, empty action output does not raise."""
    claude.run.side_effect = [
        _make_claude_result("", cost=0.94),    # action agent — empty but did work via tools
        _make_claude_result("APPROVED", cost=0.10),
    ]
...
@pytest.mark.asyncio
async def test_allow_empty_action_output_still_checks_qa(
    claude, action_agent, qa_agent, doc_store, prompt_renderer
):
    """Even with allow_empty_action_output=True, empty QA output still raises."""
    claude.run.side_effect = [
        _make_claude_result("", cost=0.94),    # action agent — empty allowed
        _make_claude_result("", cost=0.05),    # QA agent — empty NOT allowed
        _make_claude_result("", cost=0.05),    # retry also empty
    ]
