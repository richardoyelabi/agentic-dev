...
class TestRunAdoption:
    ...
    @pytest.mark.asyncio
    async def test_reverse_engineer_passes_correct_qa_output_key(self, tmp_path):
        """Ensure _reverse_engineer_spec passes qa_output_key='spec_output'
        so the QA template receives the variable it expects."""
        from agentic_dev.documents.store import DocumentStore
        from agentic_dev.orchestrator.qa_cycle import QACycleResult
        ...
        with patch(
            ...
            # Check that all spec reverse-engineering calls use qa_output_key="spec_output"
            spec_calls = [
                call for call in mock_run_qa.call_args_list
                if call.kwargs.get("output_doc_name") in (
                    "frontend_spec", "backend_spec", "api_contract"
                )
            ]
            assert len(spec_calls) == 3
            for call in spec_calls:
                assert call.kwargs.get("qa_output_key") == "spec_output", (
                    f"Expected qa_output_key='spec_output' for "
                    f"{call.kwargs['output_doc_name']}, got "
                    f"{call.kwargs.get('qa_output_key')!r}"
                )

            # Check that feature extraction uses qa_output_key="features_output"
            feature_calls = [
                call for call in mock_run_qa.call_args_list
